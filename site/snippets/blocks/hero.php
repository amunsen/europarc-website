...
<div class="relative hero w-screen bg-gray-600">
  <?php if ($block->video()->toBool() === true): ?>
    ...
    <video class="hero-video w-full h-full object-cover" preload="none" src="<?=$site->url()?>/assets/videos/<?=$block->videofile()?>"
          poster="<?=$block->image()->toFile()->url()?>"
...
?>
    <?php if ($image = $block->image()->toFile()): ?>
      <img class="w-full h-full object-cover" src="<?=$image->thumb(['format' => 'webp', 'quality' => 60])->url()?>" alt="<?=$block->headlinei() . $block->headlineii()?>">
    <?php endif;?>
  <?php endif;?>
</div>
